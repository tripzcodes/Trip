...
// TransformComponent::position. Fires its data-driven action on every entity
// that *enters* the volume (transitions outside → inside) — currently
// targeted at any entity with an `AgentComponent`. Persists the inside set
// across frames so re-entry is required to refire.
